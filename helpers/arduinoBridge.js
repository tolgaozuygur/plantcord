...
if (config.dev_mode) {
	...
	// generating fake variables
	let moisture = Math.floor(Math.random() * 101)
	let fan_state = Math.floor(Math.random() * 101)
	...
	module.exports.waterThePlant = (pump_time) => {
		console.log('waterThePlant: ' + pump_time)
	}

	module.exports.fanspeed = (fan_speed) => {
		console.log('fanspeed: ' + fan_speed)
	}
} else {
	if (config.arduino_port != "") {
		...
		console.error('It looks like you have forgotten to specify the port number on config.json. Please try again with the port number on config.json instead.')
	}
